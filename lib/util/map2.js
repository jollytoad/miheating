...
exports.mapFrom = exports.mapIf = exports.map = undefined;

var _get = require("fluxlet-immutable/get");
...
// multiplied by 2
//
const map = exports.map = iteratee => doMap(iteratee);
...
//
const mapIf = exports.mapIf = (predicate, iteratee) => doMap(iteratee, undefined, predicate);
...
//
const mapFrom = exports.mapFrom = (source, iteratee) => doMap(iteratee, source);

function doMap(iteratee, pathOrSupplier, predicate) {
  return (target, state) => {
    const source = pathOrSupplier ? (0, _get.get)(pathOrSupplier)(state) : target;

    let result;

    const applyIteratee = (init, add) => (item, index) => {
      const newItem = !predicate || predicate(item, state, index) ? iteratee(item, state, index) : item;
